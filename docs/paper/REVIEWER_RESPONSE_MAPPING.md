...
- The canonical training and evaluation entrypoint: `code/DMC_Net_experiments.py`
- The manuscript model (`phycl`) and its matched spectral baseline (`phycl_full`)
- The retained reviewer-facing support scripts:
  - `code/scripts/train_baselines.py`
  - `code/scripts/eval_noise_robustness.py`
  - `scripts/calc_complexity.py`
...
- Main accuracy and LOSO results:
  - Reproduced from `code/DMC_Net_experiments.py` with the commands listed in `README.md` and `docs/REPRODUCIBILITY.md`
...
- Baseline comparisons against general-purpose architectures:
  - Reproduced with `code/scripts/train_baselines.py`
- CPU complexity and efficiency checks discussed in the paper:
  - Supported by `scripts/calc_complexity.py` and the profiling outputs emitted by `code/DMC_Net_experiments.py`
- Noise robustness discussion:
  - Supported by `code/scripts/eval_noise_robustness.py`
